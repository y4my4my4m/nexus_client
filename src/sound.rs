...
impl SoundManager {
    pub fn new() -> Self {
        ...
        let login_success_path = PathBuf::from(base_path).join("assets/sounds/login_success_10.mp3");
        let login_failure_path = PathBuf::from(base_path).join("assets/sounds/login_error_3.mp3");
        let received_direct_message_path = PathBuf::from(base_path).join("assets/sounds/dm_5.mp3");
        let sent_direct_message_path = PathBuf::from(base_path).join("assets/sounds/dm.mp3");
        let mention_path = PathBuf::from(base_path).join("assets/sounds/mention.mp3");
        let change_channel_path = PathBuf::from(base_path).join("assets/sounds/change_channel_7.mp3");
        let send_chat_message_path = PathBuf::from(base_path).join("assets/sounds/send_chat_message.mp3");
        let receive_chat_message_path = PathBuf::from(base_path).join("assets/sounds/receive_chat_message.mp3");
        let popup_open_path = PathBuf::from(base_path).join("assets/sounds/popup_open.mp3");
        let popup_close_path = PathBuf::from(base_path).join("assets/sounds/close_1.mp3");
        let scroll_path = PathBuf::from(base_path).join("assets/sounds/load_down.mp3");
        let save_path = PathBuf::from(base_path).join("assets/sounds/save_2.mp3");
